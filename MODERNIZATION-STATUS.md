...
## Second user-reported UI pass (2026-07-21, commit 737ddb49f)
Fixed: concept-map colors in DARK mode (theme-aware CSS vars with old Taiga 2 token values), night-mode toggle (whole tuiOption row now clickable; switch presentational), course badges (Taiga 2 status names mapped to Taiga 5 appearances, old pill size/colors both themes), app-wide legacy chip-appearance CSS shim in src/scss/tui.scss, stats bar-chart crash (html-to-text options vs .map index), admin/java-view tabs (activeItemIndex is a ModelSignal — use two-way binding), header avatar pipe precedence, challenge-page mangled markup, tuiLabel on multiselects (consent/export/team-create), leaderboard sidebar negative-margin bleed.
QA infrastructure: seed script scratchpad/seed-data.py (idempotent) — students claudestudent@example.com + claudestudent2@example.com (TestPass123!), events 2-6 all types, event questions 67-73, submissions, teams 2-5, token options, goals; course 1 end date extended to 2027-01-17. ~110-page crawl as anon/teacher/student: all routes OK; remaining console-error pages are pre-existing on master (course-island skeleton has_view_permission, anon /token-values 401 redirect, dead-id problem/category pages, backend team-403 quirks).
