...
describe('AbstractEditorComponent', () => {
    ...
    it('should get form as string', () => {
        component.models = new UntypedFormArray([VariablesForm.getNewVariableForm('int')])
    ...
    it('should set form from string', () => {
        component.models = new UntypedFormArray([VariablesForm.getNewVariableForm('choice')])
        const formString = '{\n  "type": "choice",\n  "name": "test",\n  "choice": "choice",\n  "values": ["1"]\n}'
        component.setFormFromString(component.models.at(0), formString)
        expect((component.models.at(0) as UntypedFormGroup).controls.name.value).toBe('test')
        expect((component.models.at(0) as UntypedFormGroup).controls.choice.value).toBe('choice')
        expect((component.models.at(0) as UntypedFormGroup).controls.values.value[0]).toBe('1')
    })
})
